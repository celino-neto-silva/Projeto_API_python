...
class LivroBase(BaseModel):
    ...
    titulo: str = Field(title="Título do Livro",
                        description="Nome completo do livro", min_length=3)
    autor: str = Field(
        title="Autor", description="Nome completo do autor", min_length=3)
    ano_publicacao: int = Field(
        ge=1900, le=ano_atual, description="Ano em que o livro foi publicado")
